...
{
  nh->param("kSurfaceCloudDwzLeafSize", kSurfaceCloudDwzLeafSize, kSurfaceCloudDwzLeafSize);
  nh->param("kCollisionCloudDwzLeafSize", kCollisionCloudDwzLeafSize, kCollisionCloudDwzLeafSize);
  nh->param("keypose_graph/kAddEdgeCollisionCheckRadius", kKeyposeGraphCollisionCheckRadius, kKeyposeGraphCollisionCheckRadius);
  nh->param("keypose_graph/kAddEdgeCollisionCheckPointNumThr", kKeyposeGraphCollisionCheckPointNumThr, kKeyposeGraphCollisionCheckPointNumThr);
  nh->param("kDoorCollisionCheckRadius", kDoorCollisionCheckRadius, kDoorCollisionCheckRadius);
  nh->param("kDoorCollisionCheckPointNumThr", kDoorCollisionCheckPointNumThr, kDoorCollisionCheckPointNumThr);
  nh->param("kKeyposeCloudStackNum", kKeyposeCloudStackNum, kKeyposeCloudStackNum);
  nh->param("kPointCloudRowNum", kPointCloudRowNum, kPointCloudRowNum);
  nh->param("kPointCloudColNum", kPointCloudColNum, kPointCloudColNum);
  nh->param("kPointCloudLevelNum", kPointCloudLevelNum, kPointCloudLevelNum);
  nh->param("kMaxCellPointNum", kMaxCellPointNum, kMaxCellPointNum);
  nh->param("kPointCloudCellSize", kPointCloudCellSize, kPointCloudCellSize);
  nh->param("kPointCloudCellHeight", kPointCloudCellHeight, kPointCloudCellHeight);
  nh->param("kPointCloudManagerNeighborCellNum", kPointCloudManagerNeighborCellNum, kPointCloudManagerNeighborCellNum);
  nh->param("kCoverCloudZSqueezeRatio", kCoverCloudZSqueezeRatio, kCoverCloudZSqueezeRatio);
  nh->param("kUseFrontier", kUseFrontier, kUseFrontier);
  nh->param("kFrontierClusterTolerance", kFrontierClusterTolerance, kFrontierClusterTolerance);
  nh->param("kFrontierClusterMinSize", kFrontierClusterMinSize, kFrontierClusterMinSize);
  nh->param("kUseCoverageBoundaryOnFrontier", kUseCoverageBoundaryOnFrontier, kUseCoverageBoundaryOnFrontier);
  nh->param("kUseCoverageBoundaryOnObjectSurface", kUseCoverageBoundaryOnObjectSurface, kUseCoverageBoundaryOnObjectSurface);
  ...
  double sensor_range = ([&](){ double _v=0.0; nh->param("kSensorRange", _v, _v); return _v; }());
...
PlanningEnv::PlanningEnv(ros::NodeHandle* nh, std::string world_frame_id)
...
bool PlanningEnv::CheckLineOfSightInOccupancyGrid(const Eigen::Vector3i& start_pos, 
  ...
  if (!rolling_occupancy_grid_) {
    ROS_WARN("Rolling occupancy grid not initialized");
    return false;
  }
